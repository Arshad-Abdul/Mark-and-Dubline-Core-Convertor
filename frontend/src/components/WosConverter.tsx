...
const DC_ELEMENTS = [
  { key: 'dc.title',                  label: 'Article Title (TI)',                       desc: 'Item title' },
  { key: 'dc.contributor.author',     label: 'Authors / Full Names (AU)',                desc: 'Delimited with ||' },
  { key: 'dc.subject',                label: 'Keywords (Author DE + Plus ID)',           desc: 'Delimited with ||' },
  { key: 'dc.description.abstract',   label: 'Abstract, Conf., Funding',                 desc: 'Merged & sanitized' },
  { key: 'dc.publisher',              label: 'Publisher (PU)',                           desc: 'Publisher name' },
  { key: 'dc.date.issued',            label: 'Publication Year (PY)',                    desc: 'Year (YYYY)' },
  { key: 'dc.type',                   label: 'Document Type (DT)',                       desc: 'Article, Review, etc.' },
  { key: 'dc.identifier.doi',         label: 'DOI (DI)',                                 desc: 'Normalized DOI' },
  { key: 'dc.identifier.wos',         label: 'WoS Accession / UT',                       desc: 'WOS accession code' },
  { key: 'dc.identifier.issn',        label: 'ISSN / eISSN (SN)',                        desc: 'ISO 3297 (XXXX-XXXX)' },
  ...
  { key: 'dc.rights',                 label: 'Open Access (OA)',                         desc: 'Access designations' },
  { key: 'dc.description.provenance', label: 'Provenance: Web of Science',                desc: 'Fixed ingestion tag' },
];
    ...
    <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col items-center gap-2.5">
            ...
            <div>
              <p className="font-semibold text-slate-900 dark:text-slate-100 text-sm">
                Select or drop Web of Science export file
              </p>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                Supports Excel (.xls, .xlsx), Tab-delimited (.tsv, .txt), and CSV (.csv)
              </p>
            </div>
            <div className="flex items-center gap-1.5 mt-1">
              {['.XLSX', '.XLS', '.TSV', '.CSV'].map((ext) => (
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 divide-y divide-slate-100 dark:divide-slate-800">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-slate-100 dark:divide-slate-800 p-5 gap-5 sm:gap-6">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-800 dark:text-slate-200 mb-1">
              WoS Accession Identifier Schema
            </label>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-2.5">
              Select the registered Dublin Core field for the WoS record identifier (<span className="font-mono text-[11px]">UT</span>).
            </p>
            <div className="flex rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-1">
              {([
                ...
                <button
                  ...
                  className={`flex-1 rounded-md py-1.5 px-2 text-xs font-mono font-medium transition-all text-center
                    ${wosIdField === val
        ...
        <div className="rounded-xl bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 text-emerald-900 dark:text-emerald-100 p-4 flex flex-col gap-2">
          ...
          {state.detectedColumns.length > 0 && (
            <div className="text-xs text-emerald-800 dark:text-emerald-300/80 ml-7.5">
              <span className="font-medium">{state.detectedColumns.length} WoS source columns processed.</span>
              {missing.length > 0 && (
                <span className="text-slate-500 dark:text-slate-400 ml-1.5">
                  ({missing.length} optional reference fields were not present in this custom selection).
                </span>
              )}
            </div>
          )}
      ...
      <div className="flex items-center justify-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 text-center">
        ...
        <span>Output formatted with UTF-8 BOM, standard ISO hyphens, and <code className="font-mono text-slate-700 dark:text-slate-300">dc.description.provenance = "Web of Science"</code></span>
      </div>
    </div>
  );
}
